export const analyzeExamScore = async (imageElement) => {
  ...
  const baseHeight = 2000; // Base height used for calibration
  const scaleFactorY = height / baseHeight;
  
  // HARDCODED VALUES for 0% and 100% points (adjusted for scaling)
  const yZeroPercent = Math.round(831 * scaleFactorY); // 0% gridline y-coordinate
  const yHundredPercent = Math.round(528 * scaleFactorY); // 100% gridline y-coordinate
  ...
  // Resize the debug image to fit the screen better
  const resizedCanvas = document.createElement('canvas');
  const resizedCtx = resizedCanvas.getContext('2d');

  // Define a maximum width that fits well on screens
  const maxDisplayWidth = 1000; // Adjust this value as needed
  
  // Calculate the scale factor to maintain aspect ratio
  const scaleFactor = maxDisplayWidth / width;
  resizedCanvas.width = maxDisplayWidth;
  resizedCanvas.height = Math.floor(height * scaleFactor);
  
  // Draw the original debug canvas to the resized canvas
  resizedCtx.drawImage(debugCanvas, 0, 0, width, height, 
                    0, 0, resizedCanvas.width, resizedCanvas.height);
  
  // Use the resized canvas for the debug image URL
  const debugImageUrl = resizedCanvas.toDataURL('image/png');
